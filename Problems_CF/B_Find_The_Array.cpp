...
int32_t main()
{
    int t1,n,s0,s1; cin>>t1;
    while(t1--)
    {
        cin>>n; vi(a,n); inp(a,n);
        s0=0,s1=0;
        for(int i=0;i<n;i++) if(i%2) s1+=a[i]; else s0+=a[i];
        if(s0<s1)
        {
            f(i,n) if(i%2) cout<<a[i]<<" "; else cout<<1<<" ";
        }
        else
        {
            f(i,n) if(i%2==0) cout<<a[i]<<" "; else cout<<1<<" ";
        }
        cout<<"\n";
    }
}