...
int32_t main()
{
    int n,x,cnt=0; cin>>n; vector <pair<int,int>> a;
    for(int i=1;i<=n;i++)
    {
        cin>>x;
        a.pb({x,i});
    ...
    vector <int> pos;
    for(int i=0;i<n-1;i++)
        { if(a[i].ff==a[i+1].ff) { cnt++; pos.pb(i);}}
    if(cnt>=2)
    {
        cout<<"YES\n";
        f(i,n) cout<<a[i].ss<<" "; cout<<"\n";
        int f=1;
        f(i,n) {
            if(i==pos[0] and f) {f=0; cout<<a[i+1].ss<<" "<<a[i].ss<<" "; i++;}
            else cout<<a[i].ss<<" ";
        } cout<<"\n"; f=1;
        f(i,n)
        {if(i==pos[1] and f) { f=0; cout<<a[i+1].ss<<" "<<a[i].ss<<" "; i++;}
         else cout<<a[i].ss<<" ";
        }
    }
    else
