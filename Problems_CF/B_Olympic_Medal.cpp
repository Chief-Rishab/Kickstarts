...
int32_t main()
{
  IOS; int n,m,k,A,B,mr1=0,mp1=0,mp2=5005;
  cin>>n; vi(x,n); inp(x,n);
  cin>>m; vi(y,m); inp(y,m);
  cin>>k; vi(z,k); inp(z,k);
  f(i,n) mr1=max(mr1,x[i]);
  f(i,m) mp1=max(mp1,y[i]);
  f(i,k) mp2=min(mp2,z[i]);
  cin>>A>>B;
  cout<< setprecision(13)<< mr1*sqrt((double) (B*mp1)/(double) (A*mp2+B*mp1));
}
