...
int a=0,b=0,c=0,x,y,z,r,p1,p2,p3;

bool check_r(int mid)
{
    int sum=0;
    if((mid*a-x)>0) sum+=((mid*a-x)*p1);
    if((mid*b-y)>0) sum+=((mid*b-y)*p2);
    if((mid*c-z)>0) sum+=((mid*c-z)*p3);
...
int32_t main()
{
    ...
    cin>>x>>y>>z>>p1>>p2>>p3>>r;
    int s,e,mid;
    int ans=0;
    s=0,e=r+max(x,max(y,z));
    while(s<=e)
    {
        mid= e-(e-s)/2;
        if(check_r(mid))
