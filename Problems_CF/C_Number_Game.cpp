...
{
    ll count=0;
    if(n&1) return 1;  // if n is odd-> A wins

    while(n%2==0) {count++; n/=2;} // if n is even but of form 2^k, then F wins 
    if(n==1) return 0;

    if(count>1 and n!=1) //even with odd factors, and divisible by 4
    return 1;
