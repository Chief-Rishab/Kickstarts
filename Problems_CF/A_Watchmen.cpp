...
ll n; main()
{
    ...
    unordered_map <pair<ll,ll>,ll,hash_pair> points;
    ll l,r,ans=0;
    while(n--)
    {
        cin>>l>>r;
        ans+= x[l]++; // for each occurence add number of matching occurence before the current l
        ans+= y[r]++;
        ans-=points[{l,r}]++; //if same point, remove the duplicate added to the ans
    }
    cout<<ans;
}