...
int32_t main()
{
    ...
    if(vp>=vd)
    {cout<<0; return 0;}
    long double l=(vp*t),ad_dist; 
    int ans=0;
    while(l<c)
    {
        ad_dist= (l*vp)/(vd-vp); 
        l+=ad_dist; // Adding the dist queen runs until dragon meets her 
        if(l<c)
        {
            ans++;
            l+= vp*((l/vd) +f); // Queen runs addtion dist while dragon reaches the cave
        }
    }
    cout<<ans;
}