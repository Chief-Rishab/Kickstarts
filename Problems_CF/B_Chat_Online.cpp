...
{
  IOS;
  int f,g,p,q,l,r; cin>>p>>q>>l>>r;
  vi(a,p); vi(b,p); vi(c,q); vi(d,q); 
  f(i,p) { cin>>f>>g; a[i]=f; b[i]=g;}
  f(i,q) {cin>>f>>g; c[i]=f; d[i]=g;}
  int check,ans=0;
  for(int t=l;t<=r;t++)
  {
      check=0;
      for(int k=0;k<q and !check;k++)
      {
          for(int j=0;j<p and !check;j++)
          {
              if( !(d[k]+t<a[j] || c[k]+t>b[j]))
              check=1;
          }
      }
      if(check) ans++;
